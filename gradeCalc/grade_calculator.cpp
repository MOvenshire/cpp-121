...
int main() {
    ...
    // initialize sum and count variables to facillitate average calculation
    double examSum = 0;
    double examCount = 0;
    double final = 0;
    ...
    if (hwSum != 0) {
    	hw_average = hwSum/hwCount;
    }
    if (lwCount != 0) {
    	double lwTotalPossible = lwCount*100;
    	double lwActual = lwTotalPossible - lwIncomplete*100;
    	lw_average = lwActual/lwCount;
    }
    if (engagementSum != 0 &&(engagementSum<100)){
    	engagement = engagementSum;
    } else if(engagementSum>=100){
    	engagement = 100;
    }
